...
public class AStar {
	...
	public static double lastTime = System.currentTimeMillis(); // Tempo atual
	...
	private static double distance(Vector2i inicial, Vector2i fim) { 
		double dx = inicial.x - fim.x;
		double dy = inicial.y - fim.y;
		
		return Math.sqrt(dx*dx + dy*dy);
	}
	
	private static boolean verificarLista(List<Node> list, Vector2i vector) {
	...
	public static List<Node> findPath(World world, Vector2i start, Vector2i end) {
		...
		Node current = new Node(start, null, 0, distance(start, end));
		...
		while(openList.size() > 0) {
			...
			if (current.tile.equals(end)) {
				// Chegamos ao fim do destino
				// Conseguimos retornar as posições que o inimigo deve percorrer para chegar ao destino final
			...
			for (int i = 0; i < 9; i++) {
				...
				if (verificarLista(closedList, a) && gCost >= current.gCost) {
					continue;
				}
				
				if (!verificarLista(openList, a)) {
					openList.add(node);
